...
<div class="hidden md:flex flex-1 max-w-xl mx-6">
  <form action="{{ route('search') }}" method="get" class="relative w-full">
    <input
      ...
      name="q"
      value="{{ request('q') }}"
      placeholder="Cari game, pulsa, e-wallet…"
      class="w-full rounded-2xl bg-[#111826] border border-slate-800/70 ps-10 pe-20 py-2.5 outline-none text-sm placeholder:text-slate-500 focus:border-violet-500/60 focus:ring-2 focus:ring-violet-500/30 transition"
    >
    ...
    <button type="submit"
      class="absolute right-2 top-1/2 -translate-y-1/2 px-3 py-1.5 rounded-xl bg-violet-600 hover:bg-violet-500 text-xs font-medium">
      Cari
    </button>
  </form>
</div>
...
<nav class="hidden md:flex items-center gap-6 text-sm">
  <a href="{{ route('catalog') }}" class="hover:text-slate-50 transition">Digital Goods</a>
  <a href="{{ route('marketplace.index') }}" class="hover:text-slate-50 transition">Produk Marketplace</a>
  ...
  @guest
    <a href="{{ route('login') }}" class="px-4 py-2 rounded-xl border border-slate-800/70 hover:border-slate-700 transition">Login</a>
  ...
  <div id="mobileMenu" class="md:hidden hidden border-t border-slate-800/70 bg-[#0D1320]">
    <div class="px-4 py-3 space-y-2">
      <a href="{{ route('catalog') }}" class="block py-2">Digital Goods</a>
<a href="{{ route('marketplace.index') }}" class="block py-2">Produk Marketplace</a>
  ...
  <div class="flex gap-2 pt-2">
    <a href="{{ route('login') }}" class="flex-1 px-4 py-2 text-center rounded-xl border border-slate-800/70">Login</a>
