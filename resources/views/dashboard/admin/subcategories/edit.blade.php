...
@section('content')
  <a href="{{ route('admin.subcategories.index') }}" class="text-sm text-slate-400 hover:text-slate-200">← Kembali</a>
  ...
  <div class="mt-4 rounded-2xl border border-slate-800/70 bg-[#0E1524] p-6 max-w-2xl">
    ...
    <p class="text-slate-400 mt-1">Perbarui data subkategori.</p>

    <form method="post" action="{{ route('admin.subcategories.update', $subcategory) }}" class="mt-6 space-y-4">
      @csrf @method('PUT')
      ...
      <label class="inline-flex items-center gap-2 text-sm text-slate-300">
        <input type="checkbox" name="is_active" value="1" class="accent-violet-600" {{ old('is_active', $subcategory->is_active) ? 'checked' : '' }}>
        Aktif
      </label>
