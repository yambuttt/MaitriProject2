...
@section('content')
  ...
  <div class="reveal flex items-center justify-between gap-3 flex-wrap">
    <div class="space-y-1">
      ...
      <h1 class="text-3xl font-extrabold text-white tracking-tight">Sub Kategori</h1>
      <p class="text-sm text-slate-400 font-medium">Kelola sub-kategori spesifik untuk memetakan produk digital Anda.</p>
    </div>
  ...
  {{-- Filters --}}
  <form method="get" class="reveal mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[1fr_220px_140px] gap-3">
    <div class="relative">
      <input name="q" value="{{ $q }}" type="search" placeholder="Cari nama atau slug subkategori..."
    ...
    <select name="category_id" class="h-11 rounded-xl bg-black/40 border border-white/10 px-3.5 text-xs text-slate-300 outline-none focus:border-violet-500/50 transition-all">
    ...
    <div class="flex gap-3">
      <select name="per_page" class="h-11 w-full rounded-xl bg-black/40 border border-white/10 px-3.5 text-xs text-slate-300 outline-none focus:border-violet-500/50 transition-all">
  ...
  <div class="reveal mt-4 rounded-3xl quick-add-card p-5">
    <div class="text-[9px] font-extrabold uppercase tracking-widest text-violet-400 mb-3 block">⚡ Tambah Sub Kategori Cepat</div>
    
    <form method="post" action="{{ route('admin.subcategories.store') }}" class="grid grid-cols-1 md:grid-cols-[1fr_320px_140px] gap-4 items-end">
      @csrf
      <div class="grid grid-cols-1 sm:grid-cols-[180px_1fr] gap-3">
        <div class="space-y-1.5">
          ...
          <select name="category_id" required
                  class="h-11 w-full rounded-xl bg-black/40 border border-white/10 px-3 text-xs text-slate-300 outline-none focus:border-violet-500/50 transition-all">
        ...
        <div class="space-y-1.5">
          ...
          <input name="name" required placeholder="Contoh: Telkomsel, Mobile Legends, PLN..."
  ...
  {{-- Table List Grid --}}
  <section class="reveal mt-6 rounded-3xl subs-table-card overflow-hidden">
      ...
      <table class="w-full block md:table text-xs font-medium border-collapse">
        <thead class="hidden md:table-header-group">
          <tr class="bg-black/30 border-b border-white/5 font-extrabold text-slate-400 uppercase tracking-widest text-[10px]">
            <th class="text-left px-5 py-4">Kategori Utama</th>
            <th class="text-left px-5 py-4">Sub Kategori</th>
        ...
        <tbody class="w-full block md:table-row-group divide-y divide-white/[0.03] md:divide-y-0 space-y-4 md:space-y-0">
            ...
            <tr class="flex flex-col md:table-row border-b border-white/[0.03] hover:bg-white/[0.015] transition-all duration-300 p-5 md:p-0 gap-3.5 md:gap-0 bg-white/[0.01] md:bg-transparent rounded-2xl md:rounded-none mb-4 md:mb-0 shadow-lg md:shadow-none border border-white/5 md:border-none">
              ...
              <td class="block md:table-cell px-0 md:px-5 py-0 md:py-4">
                <div class="flex items-center justify-between md:block">
                  <span class="md:hidden text-[10px] font-extrabold uppercase tracking-wider text-slate-500">Kategori Utama</span>
                  <span class="text-slate-300 font-extrabold text-xs bg-white/5 border border-white/5 px-2.5 py-1 rounded-lg md:bg-transparent md:border-none md:p-0">
                    {{ $s->category?->name }}
                  </span>
                </div>
              </td>
              ...
              <td class="block md:table-cell px-0 md:px-5 py-0 md:py-4 border-t border-dashed border-white/5 md:border-none pt-3 md:pt-4">
                <div class="flex items-center justify-between md:block">
                  <span class="md:hidden text-[10px] font-extrabold uppercase tracking-wider text-slate-500">Sub Kategori</span>
                  <span class="text-white font-extrabold text-sm md:text-xs tracking-tight">{{ $s->name }}</span>
                </div>
              </td>
              ...
              <td class="block md:table-cell px-0 md:px-5 py-0 md:py-4 border-t border-dashed border-white/5 md:border-none pt-3 md:pt-4">
                <div class="flex items-center justify-between md:block">
                  ...
                  <span class="font-mono text-xs text-slate-400">{{ $s->slug }}</span>
                </div>
              </td>
                ...
                <div class="flex items-center justify-between md:block">
                  ...
                  <form method="post" action="{{ route('admin.subcategories.toggle', $s) }}">
                    ...
                    <button class="inline-flex items-center px-3 py-1 rounded-full text-[9px] font-extrabold uppercase tracking-wider border transition-all
                      {{ $s->is_active ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20 shadow-[0_0_10px_rgba(16,185,129,0.1)]' : 'bg-slate-500/10 text-slate-400 border border-white/5' }}">
              ...
              <td class="block md:table-cell px-0 md:px-5 py-0 md:py-4 border-t border-dashed border-white/5 md:border-none pt-3 md:pt-4 text-right">
                <div class="flex items-center justify-end gap-2">
                  <a href="{{ route('admin.subcategories.edit', $s) }}" 
                  ...
                  <form method="post" action="{{ route('admin.subcategories.destroy', $s) }}"
                        onsubmit="return confirm('Hapus subkategori ini beserta seluruh data produk di dalamnya?')">
                    @csrf @method('DELETE')
          ...
          @empty
            <tr class="flex flex-col md:table-row">
              <td colspan="5" class="px-5 py-10 text-center text-xs font-bold uppercase tracking-wider text-slate-500">
                Belum ada subkategori yang terdaftar.
              </td>
            </tr>
          @endforelse
        </tbody>
