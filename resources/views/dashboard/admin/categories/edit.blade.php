...
@section('content')
  ...
  <div class="reveal mt-6 rounded-[2rem] edit-form-card p-6 md:p-8 max-w-2xl space-y-6">
    <div>
      ...
      <h1 class="text-3xl font-extrabold text-white tracking-tight mt-1.5">Edit Kategori</h1>
      <p class="text-sm text-slate-400 font-medium">Perbarui parameter data kategori katalog Anda.</p>
    </div>
    ...
    <form method="post" action="{{ route('admin.categories.update', $category) }}" class="space-y-5">
      ...
      <div class="pt-1.5 pb-2">
        <label class="inline-flex items-center gap-2.5 text-xs font-bold text-slate-300 cursor-pointer">
          <input type="checkbox" name="is_active" value="1" class="accent-violet-600 size-4" {{ old('is_active', $category->is_active) ? 'checked' : '' }}>
          <span>Kategori Aktif & Ditampilkan di Katalog</span>
        </label>
      </div>
