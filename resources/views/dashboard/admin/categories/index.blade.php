...
@section('content')
  <div class="flex items-center justify-between gap-3 flex-wrap">
    <div>
      <h1 class="text-2xl md:text-3xl font-semibold">Kategori</h1>
      <p class="text-slate-400 mt-1">Kelola kategori untuk katalog.</p>
    </div>
    @if (session('ok'))
  ...
  {{-- Filter + per_page --}}
  <form method="get" class="mt-4 grid sm:grid-cols-2 lg:grid-cols-[1fr_140px_120px] gap-3">
    <div class="relative">
  ...
  <div class="mt-4 rounded-2xl border border-slate-800/70 bg-[#0E1524] p-4">
    <form method="post" action="{{ route('admin.categories.store') }}" class="grid md:grid-cols-[1fr_320px_120px] gap-3">
      @csrf
      <div>
        <label class="text-sm text-slate-300">Nama Kategori</label>
        <input name="name" required placeholder="cth: Game / Pulsa / Data"
    ...
    <div class="overflow-x-auto">
      <table class="min-w-full text-sm">
        ...
        <tbody>
            ...
            <tr class="border-t border-slate-800/70">
              ...
              <td class="px-4 py-3">
                <form method="post" action="{{ route('admin.categories.toggle', $c) }}">
                  ...
                  <button class="inline-flex items-center px-2 py-1 rounded-lg text-xs
                    {{ $c->is_active ? 'bg-emerald-500/10 text-emerald-300 border border-emerald-700/40' : 'bg-slate-500/10 text-slate-300 border border-slate-700/40' }}">
              ...
              <td class="px-4 py-3">
                <div class="flex items-center gap-2">
                  <a href="{{ route('admin.categories.edit', $c) }}" class="px-3 py-1.5 rounded-lg border border-slate-800/70 hover:border-slate-700">Edit</a>
          ...
          @empty
            <tr>
              <td colspan="4" class="px-4 py-10 text-center text-slate-400">Belum ada kategori.</td>
            </tr>
          @endforelse
        </tbody>
      </table>
