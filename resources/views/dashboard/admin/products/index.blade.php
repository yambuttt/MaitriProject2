  ...
  <div class="flex items-center justify-between gap-3 flex-wrap">
    ...
    <div class="flex items-center gap-2">
      ...
      <a href="{{ route('admin.products.create') }}"
        class="h-10 px-4 rounded-xl bg-violet-600 hover:bg-violet-500">Tambah</a>
      <!-- <form action="{{ route('admin.digiflazz.sync-master') }}" method="POST" class="inline">
        @csrf
        <button type="submit" class="btn btn-primary">
          Sync Master Digiflazz
        </button>
      </form>

      <form action="{{ route('admin.digiflazz.sync-variant-prices') }}" method="POST" class="inline ml-2">
        @csrf
              ...
              <td class="px-4 py-3">
                <div class="flex items-center gap-2">
                  ...
                  <a href="{{ route('admin.products.variants.index', $p) }}"
                    class="px-3 py-1.5 rounded-lg border border-slate-800/70 hover:border-slate-700">Variants</a>
