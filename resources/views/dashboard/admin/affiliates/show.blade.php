...
<div class="space-y-5">
  <div class="flex items-start justify-between gap-3 flex-wrap">
    <div>
      ...
      <p class="text-slate-500 text-xs mt-1">Kode: <span class="font-mono">{{ $user->affiliate_code ?? '-' }}</span></p>
    </div>
  ...
  <div class="grid gap-4 sm:grid-cols-3">
    <div class="rounded-2xl border border-slate-800/70 bg-[#0E1524] p-4">
      ...
      <div class="mt-2 text-2xl font-semibold text-slate-100">{{ number_format($summary['total_points_ledger']) }}</div>
      <div class="mt-1 text-xs text-slate-500">Saldo user maitri_points: {{ number_format((int)$user->maitri_points) }}</div>
    </div>
    <div class="rounded-2xl border border-slate-800/70 bg-[#0E1524] p-4">
        ...
        <tbody class="divide-y divide-slate-800/70">
          @forelse($conversions as $c)
            <tr class="hover:bg-white/5">
              <td class="px-4 py-3 text-xs text-slate-300">{{ $c->awarded_at?->format('d M Y H:i') ?? '-' }}</td>
              ...
              <td class="px-4 py-3 text-right font-semibold text-emerald-300">{{ number_format($c->points_awarded) }}</td>
            </tr>
          @empty
            <tr>
