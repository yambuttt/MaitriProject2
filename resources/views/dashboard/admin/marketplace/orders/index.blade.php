...
@section('content')
  ...
  <form method="get" class="mt-4 grid sm:grid-cols-2 lg:grid-cols-[minmax(0,2fr)_180px_140px] gap-3">
    <div class="relative">
      <input name="q" value="{{ $q }}" type="search" placeholder="Cari invoice, email, atau no HP…"
             class="w-full h-11 rounded-xl bg-[#0E1524] border border-slate-800/70 ps-10 pe-3 outline-none placeholder:text-slate-500 text-sm text-slate-100">
      <span class="absolute inset-y-0 left-3 flex items-center">
        <svg class="size-4 text-slate-500" viewBox="0 0 24 24" fill="none">
          <path d="m19 19-4-4m1-3a5 5 0 1 1-10 0 5 5 0 0 1 10 0Z" stroke="currentColor" stroke-width="1.5"
    ...
    <select name="status"
            class="h-11 rounded-xl bg-[#0E1524] border border-slate-800/70 px-3 text-sm text-slate-100">
    ...
    <button type="submit"
            class="h-11 rounded-xl bg-violet-600 hover:bg-violet-500 text-sm font-medium">
      Filter
    </button>
  </form>
