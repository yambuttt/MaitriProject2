        ...
        @if($user->is_affiliate)
                ...
                <div class="flex items-start justify-between gap-4 flex-wrap">
                    <div>
                        ...
                        <div class="mt-2 text-sm text-slate-300">
                            ...
                            Point Marketplace: <span
                                class="font-semibold">{{ (int) ($level->marketplace_points ?? 2000) }}</span>
                        </div>
                    </div>
            ...
            <div class="grid gap-4 sm:grid-cols-3">
                <div class="rounded-2xl border border-slate-800/70 bg-slate-950/70 p-4">
                    ...
                    <div class="text-[11px] text-slate-500 mt-1">Saldo point di user:
            ...
            <div class="rounded-2xl border border-slate-800/70 bg-slate-950/70 p-5 space-y-3">
                <div class="flex items-center justify-between gap-3 flex-wrap">
                    <div>
                        ...
                        <p class="text-xs text-slate-500">Tukar point jadi Saldo Maitri (instan) atau Uang Cash (butuh approval
                            admin).</p>
                    </div>
                    <div class="text-xs text-slate-400">
                        Point kamu: <span
                            class="font-semibold text-slate-100">{{ number_format((int) $user->maitri_points) }}</span>
                    </div>
                </div>
                ...
                <form method="post" action="{{ route('dashboard.affiliate.redeem') }}" class="grid gap-3 sm:grid-cols-3">
                    @csrf
                ...
                <script>
                    // Optional UX: disable phone jika wallet
                    ...
                    <table class="min-w-full text-sm">
                        ...
                        <tbody class="divide-y divide-slate-800">
                            @forelse($conversions as $c)
                                <tr>
                                    <td class="py-2 pr-3 text-slate-300 text-xs">{{ $c->awarded_at?->format('d M Y H:i') ?? '-' }}
                                    </td>
