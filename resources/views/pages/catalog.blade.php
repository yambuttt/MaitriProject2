      ...
      <div class="mt-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        @forelse($products as $p)
          ...
          <a href="{{ route('catalog.product.show', $p->slug) }}"
            ...
            <div class="mb-3 w-full overflow-hidden rounded-2xl border border-slate-800/70 bg-slate-900/70 aspect-[4/3]">
              @if($p->thumbnail)
                <img src="{{ Storage::url($p->thumbnail) }}" alt="{{ $p->name }}" class="w-full h-full object-cover">
              @else
            ...
            <div class="mt-3">
              <span
                class="inline-flex h-10 items-center justify-center rounded-2xl bg-violet-600 px-4 text-sm hover:bg-violet-500">
                Pilih
              </span>
            </div>
          </a>
