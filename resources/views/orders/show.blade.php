...
<div class="max-w-4xl mx-auto py-8 space-y-6">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex flex-col items-start sm:items-end gap-1">
            ...
            <span class="inline-flex items-center rounded-full border px-3 py-0.5 text-xs font-medium {{ $statusColor }}">
                {{ $status }}
            </span>
        </div>
    </div>
    ...
    {{-- Kartu utama --}}
    <div class="rounded-3xl border border-slate-800/80 bg-slate-900/80 p-5 sm:p-6 space-y-6 shadow-xl shadow-slate-950/40">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div class="space-y-1 text-right">
                ...
                <p class="text-xs text-slate-400">
                    Dibayar dengan <span class="font-medium text-slate-200">Saldo Maitri</span>
                </p>
            </div>
        </div>
        ...
        <div class="rounded-2xl border border-slate-800 bg-slate-950/60 px-4 py-3 text-xs text-slate-400 flex gap-3">
            <div class="mt-0.5 h-5 w-5 flex items-center justify-center rounded-full border border-slate-600 text-[10px] text-slate-200">
                i
            </div>
            <p>
                Simpan kode transaksi <span class="font-mono text-slate-200">{{ $order->code }}</span> sebagai bukti pembayaran.
